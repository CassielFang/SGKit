...
//
// References:
//   LearnOpenGL 6.PBR  –  https://learnopengl.com/PBR/
//   glTF 2.0 spec        –  metallic-roughness workflow
//
// SGKit integration notes:
//   - Light structs + uniform names match Renderer::SetFrameUniforms() exactly.
//   - The Renderer currently only sets u_Material.diffuse / .specular / .shininess.
//     For PBR you must extend Renderer::ExecuteBatch() to set the PBR uniforms below
//     (albedoMap, metallicMap, roughnessMap, normalMap, aoMap) and bind matching textures.
//   - IBL uniforms (irradianceMap / prefilterMap / brdfLUT) are optional - leave them
//     at 0 and the shader falls back to a constant ambient term.
//   - Light.diffuse is used as the radiance colour; .ambient / .specular are unused
//     for PBR (ambient comes from IBL, specular is computed by the BRDF).

// ============================================================================
// Structs  (light types mirror Components.h exactly)
// ============================================================================
...
// -- PBR Material  (slot assignment convention)
//     slot 0 = albedoMap      (sRGB -> linear in shader)
//     slot 1 = metallicMap    (R channel)
//     slot 2 = roughnessMap   (R channel)
//     slot 3 = normalMap      (tangent-space, uses derivative TBN)
//     slot 4 = aoMap          (R channel)
//     slot 5 = emissiveMap     (RGB, sRGB -> linear in shader)
...
vec3 RadianceDir(DirectionalLight light, vec3 N, vec3 V,
                 vec3 albedo, float metallic, float roughness, vec3 F0)
{
...
vec3 RadiancePoint(PointLight light, vec3 N, vec3 V, vec3 P,
                   vec3 albedo, float metallic, float roughness, vec3 F0)
{
...
vec3 RadianceSpot(SpotLight light, vec3 N, vec3 V, vec3 P,
                  vec3 albedo, float metallic, float roughness, vec3 F0)
{
    vec3 L = normalize(light.position - P);
    ...
    return CalcPBRRadiance(L, light.diffuse * atten * intensity, N, V,
...
void main()
{
    ...
    float metallic  = (u_PBR_combinedMR != 0 ? mrSample.b : mrSample.r)
                      * u_PBR_metallicFactor;
    ...
    if (u_dLightCount > 0)
        Lo += RadianceDir(u_DirectionalLight, N, V, albedo, metallic, roughness, F0);
    ...
    // -- Emissive ---------------------------------------------------------------
    vec3 emissiveCol = pow(texture(u_PBR_emissive, texCoord).rgb, vec3(2.2))
                       * u_PBR_emissiveFactor;
